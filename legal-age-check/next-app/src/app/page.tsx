...
function AgeVerificationContent() {
  ...
  useEffect(() => {
    fetch(`/legal-age-check/api/config`) // I have no idea why the basePath is required here
      .then((res) => res.json())
      .then((config) => setInvitationLink(config.invitationLink));
  }, []);
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-[#0142ff] to-[#8907ff] flex flex-col items-center justify-center relative overflow-hidden">
        ...
        >
          <Card className="p-6 bg-white/10 backdrop-blur-lg border-white/20 text-white">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-white/80">
                We only verify your age, nothing more
              </p>
            </div>
          </Card>
          <Card className="p-6 bg-white/10 backdrop-blur-lg border-white/20 text-white">
          ...
          <Card className="p-6 bg-white/10 backdrop-blur-lg border-white/20 text-white">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-white/80">
                Verify your age in minutes, reuse your credentials in seconds
              </p>
            </div>
          </Card>
        </motion.div>
        ...
        >
          <Button
            ...
            onClick={() =>
              invitationLink &&
              window.open(
                `https://app.archipels.io/presentation-requests/initialize?message=/verify-age&invitation=${invitationLink}&onboarding=quick`,
              )}
          >
            {invitationLink ? "Verify Age" : "Loading..."}
          </Button>
          <Button
            variant="outline"
      ...
      {/* GitHub FAB */}
      <motion.a
        href="https://github.com/Archipels-io/demos/tree/main/legal-age-check"
